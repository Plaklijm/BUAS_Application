    ...
    : tileSize(tileSize), rowCount(rowCount), colCount(colCount), tileMap(std::move(tileMap)), tileSets(tileSetList)
{
    ...
    for (const auto& tileSet : tileSetList)
    {
        ...
        textures.emplace_back(new Sprite(new Surface(cTemp), tileSet.tileCount));
    }
    ...
    if (collidable)
    {
        for (int i = 0; i < colCount; i++)
        {
            ...
            for (int j = 0; j < rowCount; j++)
            {
                ...
                {
                    collisionTiles.emplace_back(new Solid(vec2(j * 32.f, i * 32.f), vec2(tileSize,tileSize)));
                }
                else
                {
